...
int
ppp_options_ruim (char *userid, char *password)
{
  char str[1000];
  FILE *rfp;
...
int
ppp_pstn (char *dialno, char *userid, char *password, char *localip,
	  char *remoteip, char *netmask)
{
  char str[1000];
  FILE *rfp;
  ...
  do
      ...
      if (len > 0)
	{
	  ...
	  for (i = 0; (i < totlen && totlen >= 2); i++)
	    {
	      if (dial_buf[i - 1] == 'T' && dial_buf[i - 3] == 'E'
		  && dial_buf[i - 7] == 'C')
		flag = 1;
	    }
	}
...
int
modem_tx_data (unsigned char *txbuf, int tx_buf_len)
{
  int fd, wlen;
  fd = modemport.handle;
  if (status == CONNECTED)
...
int
modem_rx_data (unsigned char *rxbuf, int rx_buf_len)
{
  int fd, rlen;
  fd = modemport.handle;
  sleep (1);
