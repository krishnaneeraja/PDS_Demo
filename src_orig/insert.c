...
upDown (void *arg)
{
  char tempFile[100];
  char data[100];
  char dURL[100], uURL[100];

  fprintf (fp, "Before opening db\n");
  fflush (fp);


  fprintf (fp, "After opening db\n");
  fflush (fp);

  sprintf (data, "userid=%s&password=%s&date=", getVal ("server_user"),
	   getVal ("server_pwd"));

  sprintf (tempFile, "%s/%s", getVal ("FilePath"), "temp");
...
/***
		...
		setupEthernet();
***/
  getTime ("%Y%m%d", curDate);
  sprintf (tmpDir, "%s/%s", getVal ("FilePath"), curDate);
  sprintf (cmd, "mkdir %s", tmpDir);
  system (cmd);

  sprintf (dURL, "%s/%s", getVal ("remote_url"), getVal ("downloadURL"));
  sprintf (uURL, "%s/%s", getVal ("remote_url"), getVal ("uploadURL"));

  download (dURL, tempFile, data);
  uploadFile (uURL);
...
uploadFile (char *url)
{
  ...
  sprintf (fileName, "%s/update_%s.csv", tmpDir, curDate);

  fprintf (fp, "Calling genFile(%s)\n", fileName);
  fflush (fp);
  genFile (fileName);

  genZip ();

  sprintf (zipFile, "%s/%s.tar.gz", getVal ("FilePath"), curDate);

  fprintf (fp, "Calling upload(%s,%s)\n", zipFile, url);
  fflush (fp);

  sprintf (tempFile, "%s/%s", getVal ("FilePath"), "temp");

  upload (zipFile, url, curDate, tempFile);
  ...
  if (fgets (line, MAXLINE, file) != NULL)
    {
      ...
      fclose (file);
      if (strcasestr (line, "success") > 0)
	return 1;
    }

  parseCSV (tempFile, &updateAttendance);
  return 0;
}
...
int
download (char *url, char *tempFile, char *data)
{
  fprintf (fp, "Reached here.10\n");
  fflush (fp);

  fprintf (fp, "Downloading:::%s::\nwith params::%s::\ninto::%s::\n", url,
	   data, tempFile);
  fflush (fp);
  ...
  parseCSV (tempFile, &updateEmpDB);

  fprintf (fp, "Reached here.11\n");
  fflush (fp);
